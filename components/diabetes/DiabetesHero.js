...
const YOUTUBE_VIDEO_ID = '5qqte5cmnac';
...
export default function DiabetesHero({ onBookNow }) {
  ...
  return (
    <section className="bg-white pt-16 pb-16 px-4">
      <div className="max-w-5xl mx-auto text-center">
        ...
        >
          <span className="bg-brand-teal-light text-brand-teal rounded-full px-5 py-1.5 text-sm font-semibold tracking-wide">
            3-Month Diabetes Reversal Program
          </span>
        </div>
        ...
        >
          <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 leading-tight mb-5">
            Reverse Diabetes in 3 Months{' '}
            <span className="text-brand-teal">Without Depending on Metformin &amp; Insulin Forever</span>
          </h1>
          <p className="text-lg md:text-xl text-gray-600 leading-relaxed max-w-2xl mx-auto mb-8 font-light">
            Metformin and insulin only manage your blood sugar. The moment you stop, it spikes
            again. Watch Dt. Sushant explain the root cause and how the 3-phase reversal program
            brings your HbA1c down permanently.
          </p>
          <button
            onClick={onBookNow}
        ...
        >
          {stats.map(({ Icon, value, label }) => (
            <div key={label} className="flex items-center gap-3 text-left">
              <div className="w-10 h-10 bg-brand-teal-light rounded-xl flex items-center justify-center shrink-0">
