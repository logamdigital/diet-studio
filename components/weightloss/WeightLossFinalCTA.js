...
const trustPoints = [
  ...
  { Icon: CheckCircle2, text: 'Personalised Weight Loss Plan' },
  { Icon: Wifi,         text: 'Online Available' },
];

export default function WeightLossFinalCTA({ onBookNow }) {
  ...
  return (
    <section className="bg-white py-20 px-4">
      <div className="max-w-4xl mx-auto">
        <div
          ref={ref}
          className="opacity-0 translate-y-6 transition-all duration-700 bg-gradient-to-br from-orange-500 via-orange-400 to-amber-500 rounded-3xl p-10 md:p-16 text-center relative overflow-hidden"
        >
          {/* Decorative blobs */}
          ...
          <div className="relative z-10">
            ...
            <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight mb-6">
              Start Resetting Your Metabolism Today
            </h2>

            <p className="text-white/75 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed mb-10 font-light">
              Don&apos;t spend another year following restrictive diets that never last. Book your
              Weight Loss Consultation and get a personalised plan backed by BCA analysis to fix the
            ...
            <button
              onClick={onBookNow}
              className="bg-white text-orange-500 hover:bg-beige-100 font-bold px-10 py-5 rounded-full text-lg transition-all duration-300 shadow-2xl hover:shadow-white/20 hover:-translate-y-1 inline-flex items-center gap-3 group"
            >
            ...
            <div className="flex flex-wrap justify-center gap-8 mt-10 pt-8 border-t border-white/10">
              {[
                ['500+', 'Clients Helped'],
                ['3 Months', 'Full Reset Program'],
                ['Root Cause', 'Not Just Calorie Cutting'],
              ].map(([val, label]) => (
                <div key={label} className="text-center">
                  <p className="text-white font-bold text-2xl font-heading">{val}</p>
