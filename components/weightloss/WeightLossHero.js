...
const YOUTUBE_VIDEO_ID = 'lIzTfHhgM0c';

const stats = [
  { Icon: Clock,    value: '3 Months',           label: 'Reset Program' },
  { Icon: Activity, value: 'BCA Analysis',        label: 'Included Free' },
  { Icon: MapPin,   value: 'In-clinic + Online',  label: 'Anand / Anywhere' },
];

export default function WeightLossHero({ onBookNow }) {
  ...
  return (
    <section className="bg-white pt-16 pb-16 px-4">
      <div className="max-w-5xl mx-auto text-center">
        ...
        >
          <span className="bg-beige-100 text-orange-600 rounded-full px-5 py-1.5 text-sm font-semibold tracking-wide">
            3-Month Metabolic Weight Loss Program
          </span>
        </div>
        ...
        >
          <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 leading-tight mb-5">
            ...
            <span className="text-orange-500">Without Strict Diets or Starving Yourself Forever</span>
          </h1>
          <p className="text-lg md:text-xl text-gray-600 leading-relaxed max-w-2xl mx-auto mb-8 font-light">
            Strict diets and generic meal plans only provide temporary water weight loss. The moment
            you eat normally, the weight rushes back. Watch Dr. Sushant explain the metabolic root
            cause and how this 3-phase scientific program fixes your metabolism permanently.
          </p>
          <button
            onClick={onBookNow}
            className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-10 py-4 rounded-full text-base transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5 inline-flex items-center gap-2 group"
          >
        ...
        >
          {stats.map(({ Icon, value, label }) => (
            <div key={label} className="flex items-center gap-3 text-left">
              <div className="w-10 h-10 bg-beige-100 rounded-xl flex items-center justify-center shrink-0">
