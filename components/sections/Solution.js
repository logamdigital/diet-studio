      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2 className="section-heading mb-4">
            ...
            <span className="text-brand-purple">Actually Fit Your Life</span>
          </h2>
        ...
        {/* Conditions */}
        <div className="bg-gradient-to-br from-brand-purple-light to-brand-teal-light rounded-3xl p-8 md:p-12">
          <h3 className="font-heading text-2xl md:text-3xl font-bold text-gray-900 text-center mb-8">
...
function PillarCard({ pillar, delay }) {
  ...
  return (
    <div ref={ref} className="opacity-0 translate-y-6 transition-all duration-700">
      <div className="bg-white rounded-2xl p-6 h-full hover:shadow-md transition-shadow border border-gray-200">
        <div className="w-11 h-11 bg-brand-purple-light rounded-xl flex items-center justify-center mb-4">
          <pillar.Icon size={22} className="text-brand-purple" />
        </div>
        <div className="text-brand-purple font-bold text-xs mb-1 tracking-widest">
          STEP {pillar.step}
        </div>
        <h3 className="font-semibold text-gray-900 text-base mb-2">{pillar.title}</h3>
        <p className="text-gray-600 text-sm leading-relaxed font-light">{pillar.desc}</p>
      </div>
    </div>
  );
}
...
function ConditionBadge({ condition, delay }) {
  ...
  return (
    <div
      ref={ref}
      className="opacity-0 translate-y-4 transition-all duration-600 flex items-center gap-3 bg-white rounded-xl px-4 py-3 shadow-sm border border-gray-200 hover:border-brand-purple/30 hover:shadow-md"
    >
      <div className="w-8 h-8 bg-brand-purple-light rounded-lg flex items-center justify-center shrink-0">
        <condition.Icon size={16} className="text-brand-purple" />
      </div>
      <span className="font-medium text-gray-800 text-sm">{condition.label}</span>
    </div>
  );
}
